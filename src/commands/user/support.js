...
// ✅ FIX: import outside circular path (fallback safe)
let getReplyKeyboard;
try {
  ({ getReplyKeyboard } = require('../../utils/keyboard'));
} catch (e) {
  getReplyKeyboard = () => ({});
}
  ...
  try {
    ...
    const msg = await ctx.reply(
      `🆘 *Support*\n\n💬 Describe your issue below.\n\n⚠️ Fake, spam or illegal messages = *immediate ban*.`,
      {
        parse_mode: 'Markdown',
        reply_markup: {
          inline_keyboard: [[{ text: '🔙 Leave', callback_data: 'cb_main' }]]
        }
      }
    );

    await db.setSession(userId, STATES.SUPPORT_AWAITING_MSG, {
      lastMsgId: msg.message_id
    });

  } catch (err) {
    console.error('showSupport error:', err);
  }
}
...
async function handleSupportMessage(ctx) {
  try {
    ...
    if (!messageText && !photoFileId) {
      await ctx.reply('⚠️ Please send a text message or image.');
      return;
    }
    ...
    try {
      ...
      if (photoFileId) {
        await ctx.telegram.sendPhoto(ADMIN_ID, photoFileId, {
          ...
          reply_markup: {
            inline_keyboard: [[
              { text: '💬 Reply', callback_data: `amu_${userId}` },
              { text: '🚫 Block', callback_data: `absup_${userId}` }
            ]]
          }
        });
      } else {
    ...
    await db.setSession(userId, 'IDLE', {
      lastMsgId: msg.message_id
    });
  ...
  try {
    ...
    const msg = await ctx.reply(
      `📜 *Disclaimer*\n\n━━━━━━━━━━━━━━━━━\n✅ All coupons are *100% OFF* up to voucher amount with *NO minimum order*.\n\n📞 Contact Support for any issue.\n\n⏰ Replacements only if raised within *1–2 hours* of delivery.\n\n🚫 *No returns.*\n\n💰 Refund only if vouchers are *out of stock.*\n━━━━━━━━━━━━━━━━━`,
      {
        parse_mode: 'Markdown',
        reply_markup: {
          inline_keyboard: [[{ text: '🔙 Back', callback_data: 'cb_main' }]]
        }
      }
    );
    ...
    console.error('showDisclaimer error:', err);
  }
}
