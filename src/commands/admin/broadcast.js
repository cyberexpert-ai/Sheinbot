...
async function handleMsgUserIdInput(ctx) {
  ...
  try { await ctx.deleteMessage(); } catch (e) {}
  if (isNaN(userId)) return ctx.reply('⚠️ Invalid User ID. Send a valid Telegram ID.');
  try { await ctx.telegram.deleteMessage(ctx.chat.id, sess.data.lastMsgId); } catch (e) {}
  const msg = await ctx.reply(`💬 Send the message for user \`${userId}\`:`, {
    parse_mode: 'Markdown',
...
async function handleMsgUserTextInput(ctx) {
  ...
  try {
    ...
    const msg = await ctx.reply('❌ Failed. User may have blocked the bot.', {
      reply_markup: { inline_keyboard: [[{ text: '↩️ Back', callback_data: 'admin_broadcast_menu' }]] }
    });
    await db.setSession(ADMIN_ID, 'IDLE', { lastMsgId: msg.message_id });
    return;
...
async function showDeleteBroadcastMenu(ctx) {
  ...
  if (!broadcasts.length) {
    await ctx.answerCbQuery('No broadcasts to delete.', { show_alert: true });
    return;
  }
  const buttons = broadcasts.map(b => [{
    text: `🗑 ${(b.message || '[Photo]').slice(0, 40)}`,
    callback_data: `ab_del_${b.id}`
  }]);
  buttons.push([{ text: '↩️ Back', callback_data: 'admin_broadcast_menu' }]);
  const opts = { reply_markup: { inline_keyboard: buttons } };
  if (ctx.callbackQuery) {
    try { return await ctx.editMessageText('🗑 *Select broadcast to delete:*', { parse_mode: 'Markdown', ...opts }); } catch (e) {}
  }
